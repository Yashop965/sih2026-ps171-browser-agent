...
function resolve(targetId: number | string | undefined): Element {
  ...
  const capturedGuard =
    typeof targetId === 'number' ? getGuardForId(targetId) : getGuardForStableId(targetId);
...
function validateTypeValue(raw: unknown): string {
  ...
  if (raw.trim().length === 0) {
    throw new Error(
      'invalid TYPE value: empty - nothing typed (use KEY Backspace to clear a field)'
    );
  }
  if (raw.length > MAX_TYPE_VALUE_CHARS) {
    throw new Error(
      `invalid TYPE value: ${raw.length} chars exceeds the ${MAX_TYPE_VALUE_CHARS} cap - nothing typed`
    );
  }
  ...
  if (wanted.length >= 2) {
    const cands = options
      .filter((o) => {
        ...
        const textHit = t !== '' && (t.includes(wanted) || (t.length >= 3 && wanted.includes(t)));
...
function doKey(action: Action) {
  ...
  if (!spec && name.length === 1) {
    ...
    const code = isLetter ? `Key${c.toUpperCase()}` : isDigit ? `Digit${c}` : c; // symbols/space keep their single-char physical code
    spec = {
      code,
...
function isComboboxField(el: Element): boolean {
  ...
  const owner = el.getAttribute?.('aria-controls') || el.getAttribute?.('aria-owns');
...
async function settleFor(action: Action): Promise<void> {
  if (action.type === 'TYPE' && action.targetId !== undefined && action.targetId !== null) {
    ...
    let el: Element | undefined;
    try {
      el = typeof target === 'number' ? getElementById(target) : getElementByStableId(target);
    } catch {
      el = undefined;
    }
...
export async function execute(action: Action): Promise<ActionResult> {
  ...
  const bound =
    action.type === 'WAIT'
      ? Math.min(
          Math.max(Number.isFinite(action.waitMs) ? (action.waitMs as number) : 1000, 0),
          30_000
        ) + 500
      : ACTION_TIMEOUT_MS;

...
export function usePIIDetector() {
  ...
  const scanText = useCallback(
    (text: string): PrivacyEvent[] => {
      const detections = detectPII(text);
      const newEvents: PrivacyEvent[] = [];

      for (const det of detections) {
        newEvents.push({
          id: `det-${Date.now()}-${newEvents.length}`,
          timestamp: new Date(),
          type: 'detected',
  ...
  const redactText = useCallback(
    (text: string): { redacted: string; events: PrivacyEvent[] } => {
      const { redacted, events: detections } = redactPII(text);
      const newEvents: PrivacyEvent[] = detections.map((det, i) => ({
        id: `red-${Date.now()}-${i}`,
        timestamp: new Date(),
        type: 'redacted',
        category: det.type as PIIType,

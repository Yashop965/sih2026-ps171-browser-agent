  ...
  return (
    ...
    >
        ...
        {(() => {
          ...
          return typesToShow.map((type) => {
            ...
            const avgConfidence = Math.round(
              (items.reduce((sum, d) => sum + d.confidence, 0) / items.length) * 100
            );
              ...
              <div key={type} style={{ marginBottom: 24 }}>
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      {items.length} detected · {verifiedCount} verified · {avgConfidence}% avg
                      confidence
                    </div>
                  </div>
                  <div
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {(expandedTypes.has(type) ? items : items.slice(0, MAX_CARDS)).map(
                    (detection, index) => (
                      <div
      ...
      >
        ...
        <span>
          {detections.filter((d) => d.verified).length} of {detections.length} verified
        </span>
      </div>
    </div>
  );
